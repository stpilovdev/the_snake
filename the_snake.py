...
class Apple(GameObject):
    """Класс яблока, размещённого случайно на сетке."""
    ...
    def randomize_position(self):
        """Устанавливает объект в случайную позицию на сетке."""
        self.position = (
            randint(0, GRID_WIDTH - 1) * GRID_SIZE,
            randint(0, GRID_HEIGHT - 1) * GRID_SIZE,
        )
...
class Snake(GameObject):
    """Класс змейки, управляющейся игроком в игре."""
    ...
    def reset(self):
        """Сбрасывает состояние змейки: длину, направление и позицию."""
        self.length = 1
        self.positions = [self.position]
        self.direction = choice([UP, DOWN, LEFT, RIGHT])
        self.next_direction = None
...
def handle_keys(game_object):
    """
    Обрабатывает события клавиатуры и завершения игры.

    Изменяет направление движения игрового объекта в ответ на нажатия клавиш.
    """
...
def main():
    ...
    Инициализирует объекты, обрабатывает ввод, обновляет состояние игры
    и отрисовывает элементы на экране.
    """
